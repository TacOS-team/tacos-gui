...
namespace sombrero {
  ...
  void Label::draw() {
    ...
    // Draws the text
    pron::pronDrawText(Application::getInstance()->d, this->pronWindow, Application::getInstance()->d->defaultGC, this->getWidth() / 2 - strlen(text)/2*5, this->getHeight() / 2+5, this->text, strlen(this->text));
  }
