...
void RadioButtonGroup::add (RadioButton * radioButton) {
  if (radioButton != NULL) {
    ...
    radioButton->setGroup(this,false);
    if (radioButton->isChecked()) {
      this->setNewCheckedButton(radioButton);
    }
  }
}
...
RadioButton * RadioButtonGroup::getCheckedButton () {
  RadioButton * rb = NULL;
  if (this->radioButtons.size() > 0) {
    rb = radioButtons[this->currentCheckedButton];
  }
  return rb;
}
